...
def make_result(
    *,
    ...
    return {
        ...
        "repo": repo,
        "status": "ok" if source in {"changelog", "releases"} else ("no_data" if source == "none" else "error"),
        "source": source,
...
def build_stats(results: List[Dict[str, Any]]) -> Dict[str, int]:
    return {
        "total": len(results),
...
def build_payload(
    ...
    for result in results:
        ...
        if not include_details:
            item.pop("latest_details", None)
            item.pop("previous_details", None)
    ...
    return {
        ...
        "stats": build_stats(normalized_results),
